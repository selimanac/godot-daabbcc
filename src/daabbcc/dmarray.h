...
#define SWAP_DMARRAY(type, lhs, rhs) \
	{                                \
		type tmp = rhs;              \
		rhs = lhs;                   \
		lhs = tmp;                   \
...
void dmArray<T>::Swap(dmArray<T> &rhs) {
	SWAP_DMARRAY(T *, m_Front, rhs.m_Front);
	SWAP_DMARRAY(T *, m_End, rhs.m_End);
	SWAP_DMARRAY(T *, m_Back, rhs.m_Back);
	SWAP_DMARRAY(uint16_t, m_UserAllocated, rhs.m_UserAllocated);
}
