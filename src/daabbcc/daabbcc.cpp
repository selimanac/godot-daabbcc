...
void ErrorAssert(const char *info, uint8_t groupID) {
	godot::print_error("%s: Group ID [%i] is invalid or already removed!", info, groupID);
}

void LimitErrorAssert(const char *info, uint16_t count) {
	godot::print_error(info, " reached:", count);
}
...
void DumpQueryResult(char *title) {
	godot::print_verbose("--- %s DumpQueryResult ---", title);
	for (int i = 0; i < m_daabbcc.m_queryResult.Size(); ++i) {
		godot::print_verbose("Proxy ID: %i", m_daabbcc.m_queryResult[i]);
	}
}

void DumpManifoldResult(char *title) {
	godot::print_verbose("--- %s DumpSortResult ---", title);
	for (int i = 0; i < m_daabbcc.m_queryManifoldResult.Size(); ++i) {
		godot::print_verbose("Proxy ID: %i - Distance: %f", m_daabbcc.m_queryManifoldResult[i].m_proxyID, m_daabbcc.m_queryManifoldResult[i].m_distance);
	}
}
